...
Download dan Install QGroundControl untuk Windows dari Official Site.

Network Bypass: Saat pertama kali dibuka, Windows Defender akan menahan koneksi. Pastikan checklist Private & Public Networks diizinkan. Jika tidak, QGC akan buta jaringan dan paket MAVLink akan ter-drop.

Jalankan make px4_sitl gz_x500 di WSL/Terminal, lalu buka QGC. Koneksi UDP akan otomatis terjalin ("Vehicle Ready").

⚠️ Rules of Engagement (READ BEFORE ASKING)

Sebelum mengirim error log ke grup, pastikan kalian sudah melakukan audit mandiri:

"Gazebo saya layarnya hitam / UI nge-glitch!" -> Update driver VGA Host (Windows/Linux) kalian. WSLg merender GUI langsung melalui driver GPU Host.

"QGC tidak connect ke simulasi!" -> Cek konfigurasi Firewall. 99% kasus koneksi gagal karena packet drop di firewall.

"Error saat kompilasi / Make failed!" -> Scroll terminal ke atas, cari baris MERAH pertama yang muncul. Jangan hanya melihat pesan "Failed" di akhir. Baca log-nya.
