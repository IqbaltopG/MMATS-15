...
* **The "Twin Gate" Oscillation Dilemma:**
  When approaching the first Triple Gate, the drone was 14 meters away. At this distance, the camera captured *both* Triple Gate structures (the one we want and the one next to it). Because both were labeled identical in the YOLO dataset ("Tripple Gate"), the drone's X-error oscillated wildly between the two bounding boxes. It got stuck constantly yawing left and right (ping-pong effect) while zeroing its forward velocity because `abs(error_x) > 40`.
  * **Workaround:** Increased the force-forward threshold to `area < 80000`. This forces the drone to continuously fly forward at `0.8` m/s even while its head is oscillating left and right. As it physically gets closer, the incorrect gate naturally falls outside the camera's FOV, allowing the drone to finally lock onto just one gate.
  * **Future Training Fix (CRITICAL):** *Teliti dalam anotasi dataset!* If two visually similar but functionally distinct structures exist next to each other, they **MUST** be labeled as different classes (e.g., `Triple Gate Left` vs `Triple Gate Right`), or the model must be trained to recognize the dividing walls.
